...
import subscribeToFluxDispatcher from "./subscribeToFluxDispatcher";
import { React, ReactNative as RN, stylesheet } from "@vendetta/metro/common";

const UserStore = findByStoreName("UserStore");

if (!storage.gif_auth) {
  storage.gif_auth = { tokens: {} as Record<string, string> };
}
...
const state: AuthorizationState = {
  token: null,
  tokens: storage.gif_auth.tokens ?? {},

  init() {
    const user = UserStore?.getCurrentUser?.();
    if (!user?.id) return;

    state.tokens = storage.gif_auth.tokens ?? {};
  ...
  setToken(token: string) {
    ...
    storage.gif_auth.tokens = state.tokens;
    state.token = token;
    notify();
  },
